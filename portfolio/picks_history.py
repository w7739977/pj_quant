...
def monthly_report() -> str:
    ...
    lines = [
        ...
        "",
        "## ⚠️ 重要：本报告是「信号质量」指标，不是实盘收益",
        "",
        "**本报告所有 α / 累计 α / 胜率数字基于「5d hold」假设**：",
        "- 模型: 假设每只 picks 周一开盘买，5 个交易日后卖出",
        "- 用途: 检验模型预测信号有效性（IC 视角），与回测训练标签对齐",
        "",
        "**生产实盘策略不是 5d hold**（见 `portfolio/allocator.py:check_holdings`）：",
        "- 止损 -8% / 止盈 +15% / 持仓 20 天盘整 三种触发卖出",
        "- 实际平均持仓 几天到 20 天不等，看个股走势",
        "",
        "**所以**：",
        "- α 高 = 模型推的股 5d 后涨得比基准好 → **信号有用**",
        "- 累计 α ≠ 「如果你跟着买能赚多少」（实盘动态 hold 收益不同）",
        "- 读报告关注 **趋势** 和 **含 ST vs 排 ST 差异**，不是绝对累计数字",
        "",
        "## 术语速查",
        "",
        "- **picks**: 每周一推送的 10 只股票（生产 NUM_POSITIONS=10）",
        "- **5d 收益**: 周一开盘买入，5 个交易日后卖出的假设收益",
